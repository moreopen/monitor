 	...
 	**/
 		...
 		**/
 		function getTodayDataForever(eventCreateTime){
        	...
        	$.ajax({
 				type:"POST",
 				url:"/monitor/dataEvent/getDataForEver.htm?"+$("#requestUrl").val()+"&eventCreateTime="+eventCreateTime+"&ip="+ip,
        		dataType:"json",
 		...
 		function dayCheckLive(){
 			...
 			$(".dayCheckbox").live('click',function(){
    			var ipradio=$(":radio:checked");
    		
    			if(ipradio.length>0){
    				ip=$(":radio:checked").val();
    			}
    			...
    			if(thisObj.attr("index")==0){//表示勾选的是当天
	    			...
	    			if($(this).attr("checked")){
	    				...
	    				var thisObj=$(this);
		    				$.ajax({
				        		type:"POST",
				        		url:"/monitor/dataEvent/getOneDayData.htm?"+$("#requestUrl").val()+"&date="+$(this).val()+"&ip="+ip,
 		...
 		function appendRadio(){
         	$.ajax({
         		type:"POST",
         		url:"/monitor/ipport/getHostsByMonitorCode.htm?"+$("#requestUrl").val(),
         		success:function(msg){
         			var json=eval("("+msg+")");
         			for(var i=0;i<json.length;i++){
   	    		 ...
   	    		 if(dayObj.attr("index")==0){//表示目前操作的是今天的数据
 	    			...
 	    			chart.get("today").setData(todayData);
	      			...
	      			series.setData([null]);
   	    			 $.ajax({
   			        		type:"POST",
   			        		url:"/monitor/dataEvent/getOneDayData.htm?"+$("#requestUrl").val()+"&date="+dayObj.val()+"&ip="+ip,
   			        		success:function(msg){
   			        			if(msg=="[]"){
 		...
 		**/
 		...
 		$(document).ready(function(){
 			var requestUrl=parent.requestUrl.value;//从父页面获取请求的事件Id和itemID
 			requestUrl=requestUrl.replace("/monitor/dataEvent/dataEventPage.htm?","");
...
<div style="font-size:12px;margin:10px">tips:负数表示改时刻没有值</div>
